...
from .customs.util import fiter_overlap_bbox
...
def group_text_box(polys, slope_ths = 0.1, ycenter_ths = 0.5, height_ths = 0.5, width_ths = 1.0, add_margin = 0.05, sort_output = True):
    ...
    for poly in polys:
        ...
        if max(abs(slope_up), abs(slope_down)) < slope_ths:
            ...
            if (x_max-x_min) <20:
                x_max =int(x_max + 0.2*(x_max-x_min))
                x_min =int(x_min - 0.2*(x_max-x_min))
    ...
    horizontal_list =fiter_overlap_bbox(horizontal_list)
